...
namespace Sniffer
{
    ...
    public class IpHeader
    {
        ...
        private byte[] ByIPData = new byte[65536];

        public IpHeader(byte[] byBuffer, int startIpOffset)
        {
            try
            {
                // Create MemoryStream out of the received bytes.
                MemoryStream memoryStream = new MemoryStream(byBuffer, 14, 20);
        ...
        public string StringSourceIpAddress
        {
            get
            {
                return new IPAddress(BitConverter.GetBytes(UiSourceIPAddress)).ToString();
            }
        }

        public string StringDestIpAddress
        {
            get
            {
                return new IPAddress(BitConverter.GetBytes(UiDestinationIPAddress)).ToString();
            }
        }
